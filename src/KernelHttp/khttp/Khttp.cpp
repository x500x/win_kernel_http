...
NTSTATUS RequestSetFormBody(Request* request, const NameValuePair* pairs, SIZE_T pairCount) noexcept
{
    ...
    engine::KhNameValuePair stack[16] = {};
    engine::KhNameValuePair* apiPairs = stack;
    if (pairCount > 16) {
...
NTSTATUS RequestSetMultipartBody(Request* request, const MultipartPart* parts, SIZE_T partCount) noexcept
{
    ...
    engine::KhMultipartFormDataPart stack[16] = {};
    ...
    return engine::KhHttpRequestSetMultipartFormDataBody(detail::ToApiRequest(request), stack, partCount);
}
