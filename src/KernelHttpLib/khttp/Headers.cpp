...
namespace
{
    ...
    char* CopyText(const char* text, SIZE_T textLength) noexcept
    {
        ...
        char* copy = ::KernelHttp::AllocateNonPagedArray<char>(textLength + 1);
        if (copy == nullptr) {
            return nullptr;
        }
        if (textLength != 0) {
    ...
    {
        ::KernelHttp::FreeNonPagedArray(header.Name);
        ::KernelHttp::FreeNonPagedArray(header.Value);
        header = {};
    }
...
NTSTATUS HeadersCreate(Headers** headers) noexcept
{
    ...
    auto* created = ::KernelHttp::AllocateNonPagedObject<Headers>();
    if (created == nullptr) {
...
NTSTATUS HeadersAddEx(Headers* headers, const char* name, SIZE_T nameLength, const char* value, SIZE_T valueLength) noexcept
{
    ...
    if (slot == headers->Count && headers->Count >= ::KernelHttp::engine::KhMaxHeadersPerRequest) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }
...
void HeadersRelease(Headers* headers) noexcept
{
    ...
    headers->Count = 0;
    headers->Magic = 0;
    ::KernelHttp::FreeNonPagedObject(headers);
}
}
