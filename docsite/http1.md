...
- **禁止的额外头**：`Host`/`Content-Length`/`Connection` → `STATUS_INVALID_PARAMETER`；`Transfer-Encoding`/`TE`/`Trailer` → `STATUS_NOT_SUPPORTED`；`Expect: 100-continue` 且有 body → `STATUS_NOT_SUPPORTED`。
