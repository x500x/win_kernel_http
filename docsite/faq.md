...
A：`client::HttpsClient` 支持显式 HTTP/1.1 CONNECT 代理隧道，`KhHttpMethod` / `khttp::Method` 也有 CONNECT；高层 Session 级全局代理配置尚未暴露。TRACE 不支持。
...
A：低层 HTTP/2 已有 extended CONNECT tunnel primitive（`CONNECT` + `:protocol: websocket`，并通过 DATA 承载 WebSocket frame bytes）；高层 `kws` 仍走 HTTP/1.1 Upgrade，不会自动选择 RFC 8441。permessage-deflate 等扩展不在目标内。
...
A: `client::HttpsClient` supports explicit HTTP/1.1 CONNECT proxy tunneling, and `KhHttpMethod` / `khttp::Method` include CONNECT. Session-wide high-level proxy configuration is not exposed yet. TRACE is unsupported.
...
A: Low-level HTTP/2 has extended CONNECT tunnel primitives (`CONNECT` + `:protocol: websocket`, DATA carrying WebSocket frame bytes). High-level `kws` still uses HTTP/1.1 Upgrade and does not automatically select RFC 8441. Extensions like permessage-deflate are out of scope.
