...
| 协议 | 已支持能力 | 当前边界 |
|------|------------|----------|
| HTTP/1.1 | `Content-Length`、显式 chunked 请求体、响应 `Transfer-Encoding` 链（`chunked`/`gzip`/`deflate`/`compress`）、close-delimited 响应、HEAD/101/无 body 状态码、中间 1xx 跳过、chunked trailer 语法/禁止字段校验与只读 API 暴露、RFC 3986 相对 redirect 解析 | 用户设置请求 `Transfer-Encoding` 会被拒绝；request trailer 暂不支持；HTTP proxy/CONNECT/TRACE 不在当前主路径；`br` 仅作为 `Content-Encoding` 支持 |
...
| WebSocket | ws/wss 握手、文本/二进制发送、空消息、控制帧校验、公开 Ping/Pong/CloseEx、selected subprotocol 查询、默认接收完整消息 | 不支持扩展协商和接收分片回调；默认 API 聚合为完整消息 |
